# Mentor Service

## Стек

- Spring Boot 3
- Spring Data JDBC
- Liquibase

## Взаимодействия

Входящие:
- REST эндпоинты
- Kafka

## Схема БД

```mermaid
erDiagram
    Mentors {
        bigint id PK
        bigint mentor_telegram_user_id UK
        string telegram_url "https://t.me/zhukovsd"
        bool is_active "Выдана ли ментору роль 'MENTOR'"
    }

    Guaranteed_Reviews_Prices {
        bigint id PK
        bigint mentor_id FK
        string project_type "HANGMAN, SIMULATION, ..."
        string language "Java, Kotlin, ..."
        int price_usd 
    }

    Mentor_Descriptions {
        bigint id PK
        bigint mentor_user_id UK
        string name
        string cost "FREE, PAID, FREE_AND_PAID"
        string description
    }

    Programming_Languages {
        bigint id PK
        string name UK
    }

    Mentors_Programming_Languages {
        bigint mentor_id FK
        bigint programming_language_id FK
    }

    Services {
        bigint id PK
        string name UK
    }

    Mentors_Services {
        bigint mentor_id FK
        bigint service_id FK
    }

    Mentors ||--o{ Guaranteed_Reviews_Prices : "id = mentor_id"
    Mentors ||--o{ Mentor_Descriptions : "id = mentor_user_id"

    Mentors ||--o{ Mentors_Programming_Languages : "id = mentor_id"
    Programming_Languages ||--o{ Mentors_Programming_Languages : "id = programming_language_id"

    Mentors ||--o{ Mentors_Services : "id = mentor_id"
    Services ||--o{ Mentors_Services : "id = service_id"
```

Индексы:
- `Mentors` - уникальный индекс на значение `mentor_telegram_user_id` для проверки уникальности ментора
- `Guaranteed_Reviews_Prices` - уникальный композитный индекс на комбинацию значений `mentor_id`, `project_type`, `language`
- `Mentor_Descriptions` - уникальный индекс на значение `mentor_user_id`
- `Mentors_Programming_Languages` - композитный primary key на обе колонки
- `Mentors_Services` - композитный primary key на обе колонки
- `Programming_Languages` - `name` уникальна и case-insensitive (пример - если есть значение "Java", то "JAVA" уже не вставить)
- `Services` - `name` уникальна и case-insensitive

## Схема REST API

Для всех методов передаются [кастомные заголовки запроса](https://github.com/it-mentor-community-platform/meta/blob/main/system-analytics/services/gateway/index.md#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-security) с Telegram Id и ролями пользователя (но принимаются только необходимые в рамках конкретного запроса).

### Ответ в случае ошибки

Актуально для всех методов.

Код должен соответствовать ситуации (перечислено ниже), тело:
```
{
  "message": "Текст ошибки"
}
```

### Внутренний эндпоинт для добавления ментора

Используется при импорте менторов из Google Spreadsheet. При отсутствии профиля в profile-service создает его со ссылкой на Телеграм акаунт.

`POST /api/mentor/internal/mentor`

Тело запроса (`Content-Type: application-json`):

```
{
  "mentorTelegramUserId": 123456,
  "telegramUrl": "https://t.me/zhukovsd",
  "description": {
    "name": "Sergey Zhukov",
    "cost": "PAID",
    "description": "Мои услуги - https://zhukovsd.it/services/"
  },
  "programmingLanguages": ["Java", "Python", "Kotlin"],
  "services": ["менторство по поиску работы", "консультации"]
}
```

Ответ в случае успеха: `201 Created`.

Тело ответа содержит сохраненную сущность.

Коды ошибок:

- 400 - ошибки валидации
- 409 - ментор с таким `telegramId` уже существует  
- 500 - неизвестная ошибка

### Внутренний эндпоинт для добавления цены на гарантированное ревью

Используется при импорте цен на гарантированные ревью из Google Spreadsheet.

`POST /api/mentor/internal/guaranteed-review`

Тело запроса (`Content-Type: application-json`):

```
{
  "telegram_url": "https://t.me/zhukovsd",
  "language": "Java",
  "project_type": "SIMULATION",
  "price_usd": 15
}
```

Ответ в случае успеха: `201 Created`.

Коды ошибок:

- 400 - ошибки валидации (невалидный тип проекта, невалидная ссылка на Telegram профиль)
- 404 - профиль ментора не найден в profile-service по ссылке на Telegram акаунт
- 409 - цена на этот проект и язык программирования уже была сохранена для этого ментора
- 500 - неизвестная ошибка

### Редактирование описания текущего ментора

Используется для обновления одного или нескольких полей в описании текущего ментора

`PATCH /api/mentor/mentor/description`

Тело запроса (`Content-Type: application-json`):
```
{
  "name": "Илья Бобров",
  "cost": "PAID",
  "description": "Мои услуги - https://teletype.in/@coderilya/M-n-x49MKuy"
}
```

Ответ в случае успеха: `200 OK`. Тело - текущее состояние описания ментора (со всеми полями, а не только теми, что были переданы в PATCH запросе).

Коды ошибок:

- 400 - ошибки валидации (невалидный тип стоимости, названия, или описания)
- 404 - ментор с таким telegramId не найден
- 500 - неизвестная ошибка


### Добавление цены на гарантированное ревью текущего ментора

Используется для добавления цены на услугу гарантированного ревью.

`POST /api/mentor/guaranteed-review`

Тело запроса (`Content-Type: application-json`):
```
{
  "language": "Java",
  "project_type": "CURRENCY_EXCHANGE",
  "price_usd": 20
}
```

Ответ в случае успеха: `201 Created`. Тело - добавленное гарантированное ревью.

Коды ошибок:

- 400 - ошибки валидации
- 403 - текущий пользователь не является ментором
- 409 - у текущего ментора уже указана цена на этот тип проекта на этом языке 
- 500 - неизвестная ошибка

### Редактирование цены на гарантированное ревью текущего ментора

Используется для обновления цены существующей услуги гарантированного ревью 

`PATCH /api/mentor/guaranteed-reviews/{id}`

Тело запроса (`Content-Type: application-json`):
```
{
  "price_usd": 30
}
```

Ответ в случае успеха: `200 OK`. Тело - текущее состояние гарантированного ревью (со всеми полями, а не только тем, что было передано в PATCH запросе).

Коды ошибок:

- 400 - ошибка валидации стоимости
- 403 - текущий пользователь не является ментором, или пытается обновить цену на чужое ревью
- 404 - ревью с переданным айди не найденно
- 500 - неизвестная ошибка

## Kafka

### Consumer для топика `auth.user.authenticated`

Consumer group - `mentor-service-cg`.

Используется для актуализации поля `is_active` в таблице `Mentors`. Если у пользователя есть роль `MENTOR`, `is_mentor` присваивается значение `true`.

Payload сообщения - https://github.com/it-mentor-community-platform/meta/blob/main/system-analytics/services/auth-service/index.md#producer-%D0%B4%D0%BB%D1%8F-%D1%82%D0%BE%D0%BF%D0%B8%D0%BA%D0%B0-authuserauthenticated.

### Consumer для топика `projects.project.created`

Consumer group - `mentor-service-cg`.

Используется для уведомления менторов, делающих ревью на сданный студентом проект.

Payload сообщения - https://github.com/it-mentor-community-platform/meta/blob/main/system-analytics/services/project-service/index.md#producer-%D0%B4%D0%BB%D1%8F-%D1%82%D0%BE%D0%BF%D0%B8%D0%BA%D0%B0-projectsprojectcreated.

Пример:
- В системе есть 3 ментора:
  - Иван ревьюит все проекты на Java
  - Пётр ревьюит только виселицу и симуляцию на Java, Kotlin
  - Саша ревьюит все проекты на Python
- Студент сдаёт симуляцию на Java
- Нам необходимо уведомить Ивана и Петра

Результат работы консьюмера - новое сообщение в топик `notifications.mentors.project.submitted`

### Producer для топика `notifications.mentors.project.submitted`

Используется для уведомления менторов о сдаче студентом проекта.

Payload сообщения:

```
{
  "project": {
    // все поля из payload сообщения `projects.project.created`
  },
  "mentors": [
    {
      "mentor_telegram_user_id": 1,
      "mentor_telegram_profile_url": "https://t.me/zhukovsd" // может быть null
    }
  ]
}
```
