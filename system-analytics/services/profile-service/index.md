...
### Внутренний эндпоинт создания профиля

Используется при создании профиля для участников сообщества не попавших на таблицу студентов, например, менторы не сдававшие проекты. Поддерживает создание профиля с деталями.
...
Ответ в случае успеха: `201 Created` при успешном создании профиля.

Коды ошибок:

- 400 - ошибки валидации (невалидные поля)
- 409 - профиль с переданным `telegram_user_id` уже существует
- 500 - неизвестная ошибка
...
- 400 - ошибки валидации запроса (например, неправильно заданы параметры фильтрации и пагинации)
...
Используется для инициализации профиля пользователя в таблице `Profiles` и заполнения начальных значений для деталей `telegram_url`, `gihub_profile_url`, `first_name`, `last_name`.
