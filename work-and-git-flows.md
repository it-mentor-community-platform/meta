...
- Взяв задачу в работу, разработчик создаёт ветку в нужном репозитории. Сделать это можно через интерфейс GitHub нажав на ссылку "Create a branch" на странице назначенной задачи, в блоке Development боковой панели.
  - Если ветка создана была локально/через IDE/любым другим способом, на странице будущего pull request так же будет блок Development на боковой панели, где можно выбрать связанную с ним задачу.
  - Нейминг веток: `feature-xxx-NNN`. `xxx` - краткое описание задачи, `NNN` - номер относящейся к задаче GitHub Issue (если есть). Пример - `feature-jwt-validation-7`.
- Изменения пушатся в ветку
- По готовности задачи разработчик создаёт pull request. Правки, при наличии, допушиваются в ту же ветку
- Необходимо назначить Lead ревьюером pull request'а
